...
class MappedValuesRDD[K, V, U](prev: RDD[_ <: Product2[K, V]], f: V => U)
  ...
  override def compute(split: Partition, context: TaskContext,isRDDCache: Boolean): Iterator[(K, U)] = {
    if(isRDDCache) {
      firstParent[Product2[K, V]].iterator(split, context).map { pair => (pair._1, f(pair._2))}
    }else{
      firstParent[Product2[K, V]].iteratorK(split, context).map { pair => (pair._1, f(pair._2))}
    }
  }
}
