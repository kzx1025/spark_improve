...
class MappedValuesRDD[K, V, U](prev: RDD[_ <: Product2[K, V]], f: V => U)
  ...
  override def compute(split: Partition, context: TaskContext,shuffleMemorySignal :ShuffleMemorySignal): Iterator[(K, U)] = {
    if(shuffleMemorySignal.getIsCache) {
      firstParent[Product2[K, V]].iteratorK(split, context,shuffleMemorySignal).map { pair => (pair._1, f(pair._2))}
    }else{
      firstParent[Product2[K, V]].iteratorK(split, context,shuffleMemorySignal).map { pair => (pair._1, f(pair._2))}
    }
  }
